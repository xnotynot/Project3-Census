...
df = pd.read_csv('https://github.com/xnotynot/Project3-Census/blob/main/Data/popincome_data_plotly.csv')
...
header = html.H4(
    "Plotly with Dash Module", className="bg-primary text-white p-2 mb-2 text-center"
)
...
dropdown = html.Div(
    [
        ...
        dcc.Dropdown(
            ["MedianAge","PerCapitaIncome","Population","PovertyCount"],
            "Population",
            id="indicator",
            clearable=False,
        ),
...
def update_line_chart(indicator, state, yrs, theme):
    if state == [] or indicator is None:
        return {}, {}, []
    ...
    fig_scatter = px.scatter(
        df.query(f"year=={yrs[1]} & continent=={state}"),
        x="year",
        y="Population",
        size="pop",
        color="state",
